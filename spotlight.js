class SPOTLIGHT {
    ...
    constructor(el, x, y, r=500) {
        ...
        this._raycast = new this._raycastClass(el.width, el.height);
    }

    addRect(x, y, width, height) {
        this._raycast.addRect(x, y, width, height);
    }
    ...
    drawRects() {
        ...
        this._raycast._rect_list.forEach(r => {drawRect(r.x, r.y, r.width, r.height)});
    }

    drawCasts(drawLines=false) {
        let casts = this._raycast.getCasts(this.x, this.y);
        this.drawLighting(casts);
        if(drawLines)
            casts.map(c => {this.drawCast(c.x, c.y)});
    }

    drawCast(x, y, color="red") {
    ...
    drawLighting(casts, light="transparent", shadow="#111111") {
        this.doPolygon(casts, shadow, true, false);
    
        let gradient = this._context.createRadialGradient(this.x, this.y, 0, this.x, this.y, this.r);
        gradient.addColorStop(0, light);
        gradient.addColorStop(1, shadow);
        this.doPolygon(casts, gradient, false, false);
    }

    doPolygon(casts, style, invert=false, blurring='10px') {
        this._context.beginPath();
        this._context.moveTo(casts[0].x, casts[0].y);
        if(invert) {
            ...
            this._context.lineTo(0, 0);
            this._context.lineTo(casts[0].x, casts[0].y);
        }
        for(let c in casts) {
            if(c === 0) continue;
            this._context.lineTo(casts[c].x, casts[c].y);
        }
        ...
        if(blurring) {
            this._context.filter = `blur(${blurring})`;
            this._context.fill(invert? "evenodd": undefined);
        }

        this._context.filter = 'blur(0)';
        this._context.fill(invert? "evenodd": undefined);
    }

    drawSpotlight(){
        this._context.beginPath();
        this._context.arc(this.x, this.y, 15, 0, 2 * Math.PI);
        this._context.fillStyle = "#ffff77";
        this._context.fill();
        this._context.strokeStyle = "#ffffaa";
        this._context.stroke();
    }
}
